...
Usage:
    ...
    python live_transcribe_sd.py --device 7      # prompts you to pick en/es/auto at startup
    python live_transcribe_sd.py --device 7 --language es      # skip the prompt, force Spanish
    python live_transcribe_sd.py --device 7 --no-prompt         # skip the prompt, auto-detect
...
def main():
    ...
    parser.add_argument("--language", default=None,
                         help="Language code (en, es) or 'auto'. If omitted, you'll be prompted at startup.")
    parser.add_argument("--no-prompt", action="store_true",
                         help="Skip the interactive language prompt and just auto-detect (useful for scripted runs).")
    ...
    # Normalize "auto" (or an empty --language) to None, which faster-whisper
    # treats as auto-detect.
    if args.language and args.language.strip().lower() in ("", "auto"):
        args.language = None

    if args.language is None and not args.no_prompt:
        choice = input("Language for this session? [en/es, or press Enter for auto-detect]: ").strip().lower()
        if choice in ("en", "es"):
            args.language = choice
        elif choice:
            print(f"'{choice}' not recognized — using auto-detect.")
