...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Force a specific mic by PortAudio index (see list_mics.py).",
    )
    parser.add_argument(
        "--silero-sensitivity",
        type=float,
        default=0.4,
        help="Voice detection sensitivity, 0-1. Higher = picks up speech more easily "
        "(but more false positives). Default: 0.4.",
    )
    parser.add_argument(
        "--webrtc-sensitivity",
        type=int,
        default=3,
        choices=[0, 1, 2, 3],
        help="WebRTC VAD aggressiveness, 0-3. Lower = less strict about what counts "
        "as speech. Default: 3 (strictest).",
    )
